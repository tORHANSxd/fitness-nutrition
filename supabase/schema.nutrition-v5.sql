-- V5 catalog review supplement, generated by isolated PGlite tests.
...
CREATE OR REPLACE FUNCTION public.activate_plan_protocol_v1(p_config jsonb, p_expected_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
-- ACL: {postgres=X/postgres,authenticated=X/postgres}
CREATE OR REPLACE FUNCTION public.guard_nutrition_snapshots_v1()
 RETURNS trigger
 LANGUAGE plpgsql
 SET search_path TO ''
AS $function$
...
-- ACL: {postgres=X/postgres,authenticated=X/postgres}
CREATE OR REPLACE FUNCTION public.nutrition_goals_capabilities_v1()
 RETURNS integer
 LANGUAGE sql
 IMMUTABLE
 SET search_path TO ''
...
-- ACL: {postgres=X/postgres,authenticated=X/postgres}
CREATE OR REPLACE FUNCTION public.nutrition_number_v1(v jsonb, minimum numeric DEFAULT 0, exclusive boolean DEFAULT false)
 RETURNS numeric
 LANGUAGE plpgsql
 IMMUTABLE
 SET search_path TO ''
...
-- ACL: {postgres=X/postgres,authenticated=X/postgres}
CREATE OR REPLACE FUNCTION public.validate_nutrition_snapshot_v1(s jsonb)
 RETURNS boolean
 LANGUAGE plpgsql
 STABLE
 SET search_path TO ''
...
begin
  ...
  if p_profile->>'targetMode' = 'calibrated' then
    ...
    if not public.validate_tre_protocol_v2(snapshot) or p_profile->>'allocationMode' is distinct from 'explicitMacros'
...
-- ACL: {postgres=X/postgres,authenticated=X/postgres}
CREATE OR REPLACE FUNCTION public.validate_tre_protocol_v2(p jsonb)
 RETURNS boolean
 LANGUAGE plpgsql
 STABLE
 SET search_path TO ''
...
-- user_plan_protocols.user_plan_protocols_config_check: CHECK ((validate_tre_protocol_v2(config) AND (schema_version = ((config ->> 'schemaVersion'::text))::smallint)))
-- user_plan_protocols.user_plan_protocols_schema_version_check: CHECK ((schema_version = ANY (ARRAY[1, 2])))
CREATE TRIGGER protect_nutrition_actual BEFORE INSERT OR UPDATE ON public.daily_checkins FOR EACH ROW EXECUTE FUNCTION guard_nutrition_snapshots_v1();
CREATE TRIGGER protect_nutrition_plan BEFORE INSERT OR UPDATE ON public.daily_plans FOR EACH ROW EXECUTE FUNCTION guard_nutrition_snapshots_v1();
